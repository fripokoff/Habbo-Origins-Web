...
function highlightFlag(countryCode) {
	...
	const specificFlag = document.querySelector(`.fi-${countryCode}`);
	if (specificFlag) {
		specificFlag.style.transform = 'scale(1.5)';
	}
}


function setLoaderbyCouuntryCode(countryCode) {
	localStorage.setItem('origins_hotel', countryCode);
	highlightFlag(countryCode);
